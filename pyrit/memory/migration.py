...
def _build_initial_metadata() -> MetaData:
    """
    Build a MetaData reflecting the initial pre-Alembic baseline schema.

    Runs only the root revision (``_INITIAL_REVISION``) against a temporary
    in-memory SQLite database and reflects the result.  This avoids duplicating
    column definitions and gives an accurate baseline to compare unversioned
    legacy databases against.

    Returns:
        MetaData: Reflected metadata from the initial schema.
    """
    temp_engine = create_engine("sqlite:///:memory:")
    with temp_engine.begin() as temp_conn:
        temp_config = _make_config(connection=temp_conn)
        command.upgrade(temp_config, _INITIAL_REVISION)
    meta = MetaData()
    with temp_engine.connect() as temp_conn:
        meta.reflect(temp_conn, only=list(_MEMORY_TABLES))
    return meta
...
def _validate_and_stamp_unversioned_memory_schema(*, config: Config, connection: Connection) -> None:
    ...
    try:
        ...
        migration_context = MigrationContext.configure(
            ...
            opts={"compare_type": False, "include_name": _include_name_for_memory_schema},
        )

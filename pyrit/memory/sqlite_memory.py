...
class SQLiteMemory(MemoryInterface, metaclass=Singleton):
        ...
        *,
        ...
        with closing(self.get_session()) as session:
            try:
                ...
                logger.exception(f"Error fetching data from table {model_class.__tablename__}: {e}")  # type: ignore[unresolved-attribute]
                raise
        ...
        with closing(self.get_session()) as session:
            try:
                for entry in entries:
                    ...
                    entry_in_session = session.get(type(entry), entry.id)  # type: ignore[unresolved-attribute]
    ...
    def export_all_tables(self, *, export_type: str = "json") -> None:
        ...
        for model in table_models:
            ...
            self.exporter.export_data(list(data), file_path=file_path, export_type=export_type)  # type: ignore[invalid-argument-type]

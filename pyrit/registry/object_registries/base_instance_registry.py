...
.. note::
    ...
    **Legacy stack — do not build new registries on this.** New component
    ...
    ``ConverterRegistry`` for the target shape. This class remains only
    because ``AttackTechniqueRegistry`` still subclasses it; it is removed
    once that migrates.
...
where callers retrieve stored objects directly, use ``Registry`` + the
``.instances`` property (``DefaultInstanceRegistry``) instead.
...
class BaseInstanceRegistry(ABC, RegistryProtocol[ComponentIdentifier], Generic[T]):
    ...
    .. note::
        ...
        **Legacy — do not subclass for new registries.** New component
        ...
        survives only for the not-yet-migrated ``AttackTechniqueRegistry`` and
        is removed once it moves to ``.instances``.

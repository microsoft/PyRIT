...
def _is_content_filter_error(data: Union[dict[str, object], str]) -> bool:
    ...
    if isinstance(data, dict):
        ...
        code = error_obj.get("code") if isinstance(error_obj, dict) else None  # type: ignore[invalid-argument-type]
        ...
        if code == "invalid_prompt":
            message = error_obj.get("message", "") if isinstance(error_obj, dict) else ""  # type: ignore[no-matching-overload]

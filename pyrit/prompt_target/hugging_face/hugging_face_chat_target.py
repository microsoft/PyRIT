...
from transformers import (
    AutoModelForCausalLM,  # type: ignore[possibly-missing-import]
    AutoTokenizer,  # type: ignore[possibly-missing-import]
    BatchEncoding,
    PretrainedConfig,
)
...
class HuggingFaceChatTarget(PromptChatTarget):
    ...
    async def load_model_and_tokenizer(self) -> None:
        ...
        try:
            ...
            self.model = self.model.to(self.device)  # type: ignore[invalid-argument-type]
    ...
    async def _send_prompt_to_target_async(self, *, normalized_conversation: list[Message]) -> list[Message]:
        ...
        try:
            ...
            assistant_response = cast(
                "str",
                self.tokenizer.decode(generated_tokens, skip_special_tokens=self.skip_special_tokens),  # type: ignore[unresolved-attribute]
            ).strip()

        ...
        with open(csv_file_path, newline="") as csvfile:
            ...
            for row in csvreader:
                ...
                message_piece = MessagePiece(
                    role=row["role"],  # type: ignore[invalid-argument-type]
                    original_value=row["value"],
                    original_value_data_type=row.get("data_type", None),  # type: ignore[invalid-argument-type]
                    conversation_id=row.get("conversation_id", None),
                    sequence=int(sequence_str) if sequence_str else 0,
                    labels=labels,  # deprecated
                    response_error=row.get("response_error", None),  # type: ignore[invalid-argument-type]

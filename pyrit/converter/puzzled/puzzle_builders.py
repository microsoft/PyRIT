...
def _grid_size(words: list[str]) -> int:
    ...
    # PUZZLED sizing (Ahn & Lee, arXiv:2508.01306): the grid must fit the longest word with
    # margin, and scale with the number of words so placement does not thrash.
...
def build_word_search(words: list[str], rng: random.Random) -> str:
    ...
    words cannot be placed in the initial grid the grid grows by one and the attempt
    restarts, so the builder always terminates for reasonable inputs.
    ...
    while True:
        ...
        # Longest words first: they are the hardest to fit, so place them while the grid
        # is still empty.
        placed = all(_try_place(grid, word, rng) for word in sorted(normalized, key=len, reverse=True))
        if placed:
            break
        size += 1

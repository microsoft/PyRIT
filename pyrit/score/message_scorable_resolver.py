...
class MessageScorableResolver:
    ...
    def resolve(self, *, scorable: Scorable, memory: MemoryInterface) -> Message:
        ...
        print_deprecation_message(
            old_item="MessageScorableResolver.resolve",
            new_item="MessageScorableResolver.resolve_async",
            removed_in="1.4.0",
        )
    ...
    @legacy_sync_override(lambda: MessageScorableResolver.resolve)
    async def resolve_async(self, *, scorable: Scorable, memory: MemoryInterface) -> Message:
        """
        Resolve supported scorables to the message view consumed by message scorers.
        ...
        """
        if isinstance(scorable, MessageScorable):
            return await self._resolve_message_reference_async(scorable=scorable, memory=memory)
        if isinstance(scorable, ContentEntryScorable):
            return self._adapt_content(
                scorable=(await self._resolve_content_reference_async(scorable=scorable, memory=memory))
            )
        if isinstance(scorable, ContentScorable):
            return self._adapt_content(scorable=scorable)
        raise TypeError(
    ...
    @staticmethod
    async def _resolve_content_reference_async(
        *, scorable: ContentEntryScorable, memory: MemoryInterface
    ) -> ContentScorable:
        content = (await memory.get_scorable_content_async(content_ids=[scorable.content_id])).get(scorable.content_id)
        if content is None:
            raise ValueError(f"No stored scorable content found for id {scorable.content_id}.")
        return content
    ...
    @staticmethod
    async def _resolve_message_reference_async(*, scorable: MessageScorable, memory: MemoryInterface) -> Message:
        pieces = await memory.get_message_pieces_async(prompt_ids=list(scorable.message_piece_ids))
        wanted = {str(piece_id) for piece_id in scorable.message_piece_ids}
        pieces = [piece for piece in pieces if str(piece.id) in wanted]
        found = {str(piece.id) for piece in pieces}
        missing = [str(piece_id) for piece_id in scorable.message_piece_ids if str(piece_id) not in found]

...
async def initialize_pyrit_async(
    ...
    **memory_instance_kwargs: Any,
    ...
    if memory_db_type == IN_MEMORY:
        ...
        memory = SQLiteMemory(db_path=":memory:", **memory_instance_kwargs)  # type: ignore[invalid-assignment]
    elif memory_db_type == SQLITE:
        logger.info("Using persistent SQLite database.")
        memory = SQLiteMemory(**memory_instance_kwargs)  # type: ignore[invalid-assignment]
    elif memory_db_type == AZURE_SQL:
        logger.info("Using AzureSQL database.")
        memory = AzureSQLMemory(**memory_instance_kwargs)  # type: ignore[invalid-assignment]
    else:
        raise ValueError(

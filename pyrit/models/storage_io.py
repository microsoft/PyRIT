...
"""
Deprecation shim — the storage I/O classes moved to ``pyrit.io.storage``.

Importing names from ``pyrit.models.storage_io`` still works for one release but
emits a one-time ``DeprecationWarning`` per name. Import from ``pyrit.io`` instead.
This shim will be removed in 0.17.0.
"""
...
__getattr__ = module_deprecation_getattr(
    ...
    target_module="pyrit.io.storage",
    names=__all__,
    removed_in="0.17.0",
)

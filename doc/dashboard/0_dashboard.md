...
The `.azuredevops/adversarial-benchmark.yml` pipeline can run the scenario, upsert the
committed store, and open a PR with the change automatically — set its `publishBenchmarkStore`
parameter to `true` (requires a `GITHUB_PAT` secret pipeline variable; see the comment at the
top of that file). This still only updates the JSONL store — regenerating the HTML fragments
(step 2 below) remains a separate, manual step after that PR merges. To do the whole flow by
hand instead:
...
Scorer Quality and the HTML fragment regeneration step are still fully manual today. Only the
Benchmark Leaderboard's store-update step has opt-in CI automation, as described above.

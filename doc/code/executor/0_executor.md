...
## Attack outcomes and scenario completion

Attack execution has two independent axes:
...
flowchart TB
    ...
    subgraph aggregation["AtomicAttack and Scenario aggregation"]
...
Scenario retries resume only objectives that have not completed; already-persisted results are
preserved. If retry exhaustion leaves any incomplete objectives, `ScenarioPartialFailureException`
reports `completed_count`, `incomplete_count`, and `incomplete_objectives`, and keeps the first
objective exception as its cause. This typed exception is also used when **none** of the objectives in
the returned `AttackExecutorResult` completed (`completed_count == 0`). If an `AtomicAttack` raises
before it can return an `AttackExecutorResult`, the scenario instead retries and ultimately re-raises
that exception; multiple concurrent atomic-attack failures are surfaced as an `ExceptionGroup`.

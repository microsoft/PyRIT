...
- A scorer declares which evidence it reads, rather than the caller filtering evidence for it. A `MessageScorer` states the conversation roles it reads on its `ScorerPromptValidator`, and stays silent when the evidence carries no role it reads.
- Readability is judged after the scorer acquires its own evidence, not before it is called. A scorer that never reads the response must still run when the response failed.
- `Score.status` marks a verdict complete or undetermined, and the attack decides how to branch on it. Evidence a scorer cannot read reports undetermined, so "no verdict was reachable" is never confused with a negative verdict.

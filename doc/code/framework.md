...
- We need to move some older attacks that don't belong here. Many should just be attack techniques (`FlipAttack` has been migrated — it is now the `flip` core technique built from `PromptSendingAttack` plus a `TaskFramingConverter` + `FlipConverter` stack)

...
class TestSendJsonWithRetryAsync:
    ...
    async def test_retry_then_success_rolls_back_poisoned_turn(self):
        ...
        pieces = await normalizer.memory.get_message_pieces_async(conversation_id=conversation_id)
    ...
    async def test_no_response_raises_value_error(self):
        ...
        normalizer.memory = get_mock_prompt_normalizer()
        normalizer.memory.get_message_pieces_async = AsyncMock(return_value=[])
        normalizer.memory.delete_conversation_pieces_after_sequence_async = AsyncMock(return_value=0)

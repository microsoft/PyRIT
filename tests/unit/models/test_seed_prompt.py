...
# --- response_json_schema resolution (response_json_schema_name is InitVar-only) ---


class TestSeedPromptResponseJsonSchemaResolution:
    """Tests covering how SeedPrompt resolves an embedded JSON schema.

    ``response_json_schema_name`` is a dataclass ``InitVar`` — accepted as a
    constructor kwarg / YAML key and resolved during ``__post_init__`` into
    ``response_json_schema``, but **not** stored as an instance attribute.
    Downstream readers (scorers, memory, attacks) only see
    ``response_json_schema``.
    """

    def test_name_is_not_a_dataclass_field(self):
        """Regression guard: ``response_json_schema_name`` must remain an InitVar.

        If a future change converts it back to a regular field, this test
        breaks loudly so we catch the leak before it propagates into memory
        persistence or scorer identifier params.
        """
        field_names = {f.name for f in dataclasses.fields(SeedPrompt)}
    ...
    def test_inline_schema_left_unchanged(self):
        ...
        # InitVar is consumed by __post_init__; it must not land on the instance.
    ...
    def test_name_resolves_against_registry(self):
        ...
        # InitVar is not stored on the instance — only the resolved schema is.
        assert "response_json_schema_name" not in sp.__dict__

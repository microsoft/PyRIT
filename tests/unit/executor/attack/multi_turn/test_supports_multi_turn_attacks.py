...
async def _seed_conversation_async(*, conversation_id: str, system_prompt: str, user_text: str = "Hello") -> None:
...
class TestRotateConversationForSingleTurnTarget:
    ...
    async def test_pending_first_turn_context_suppresses_rotation_after_prepended_turns(self):
        strategy = _make_strategy(supports_multi_turn=False)
        context = _make_context()
    ...
    async def test_rotates_on_second_turn_for_single_turn_target(self):
        strategy = _make_strategy(supports_multi_turn=False)
        context = _make_context()
        context.executed_turns = 1
        original_id = context.session.conversation_id
    ...
    async def test_rotates_multiple_turns(self):
        strategy = _make_strategy(supports_multi_turn=False)
        context = _make_context()
...
class TestSystemPromptCarryoverOnRotation:
    ...
    async def test_system_prompt_duplicated_into_new_conversation(self):
        ...
        memory = CentralMemory.get_memory_instance()
        new_messages = await memory.get_conversation_messages_async(conversation_id=new_id)
    ...
    async def test_system_prompt_preserved_across_multiple_rotations(self):
        ...
        for turn in range(1, 4):
            ...
            messages = await memory.get_conversation_messages_async(conversation_id=context.session.conversation_id)
    ...
    async def test_no_system_prompt_yields_fresh_conversation_id(self):
        ...
        new_messages = await memory.get_conversation_messages_async(conversation_id=context.session.conversation_id)
        assert len(new_messages) == 0

    async def test_user_messages_not_carried_over(self):
    ...
    async def test_multiple_system_messages_all_carried_over(self):
    ...
    async def test_empty_conversation_yields_fresh_id(self):
    ...
    async def test_only_system_messages_all_carried_over(self):
        ...
        (await memory.add_message_pieces_to_memory_async(message_pieces=[sys_piece]))
    ...
    async def test_multipiece_system_message_fully_duplicated(self):
    ...
    async def test_old_conversation_untouched_after_rotation(self):
        ...
        old_messages = await memory.get_conversation_messages_async(conversation_id=old_id)
...
class TestTAPNodeDuplicateSystemMessages:
    ...
    async def test_single_turn_target_duplicates_logical_history_without_seed_boundary(self):
    ...
    async def test_multi_turn_target_duplicates_full_conversation(self):
    ...
    async def test_single_turn_no_system_messages_retains_full_history(self):
    ...
    async def test_adversarial_chat_always_fully_duplicated(self):
    ...
    async def test_single_turn_multiple_system_messages_and_user_are_duplicated(self):
    ...
    async def test_single_turn_empty_conversation_yields_fresh_id(self):
    ...
    async def test_system_message_content_preserved_exactly(self):
    ...
    async def test_original_conversation_untouched_after_duplicate(self):
    ...
    async def test_single_turn_multipiece_system_message_duplicated(self):
...
class TestTAPBranchingPreservesSystemPrompts:
    ...
    async def test_branching_single_turn_target_preserves_system_across_depths(self):
        ...
        (await memory.add_message_pieces_to_memory_async(message_pieces=[user2, asst2]))

        # Verify branch1 now has the inherited and new turns.
        branch1_full = await memory.get_conversation_messages_async(
            conversation_id=branch1.objective_target_conversation_id
        )
    ...
    async def test_branching_multi_turn_target_preserves_full_history(self):

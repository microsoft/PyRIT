...
def test_init_skips_prod_guard_when_skip_schema_migration_true():
    """When skip_schema_migration=True, the prod guard is bypassed entirely — no error, no migration."""
    prod_conn = "Server=tcp:prod.database.windows.net;Database=prod_db;"
    saved = Singleton._instances.copy()
    Singleton._instances.clear()
    try:
        with (
            ...
            # Should not raise even though connection matches prod
            AzureSQLMemory(
                connection_string=prod_conn,
                results_container_url="https://test.blob.core.windows.net/test",
                results_sas_token="valid_sas_token",

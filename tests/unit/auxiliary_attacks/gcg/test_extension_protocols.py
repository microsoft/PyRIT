...
"""Tests for :mod:`pyrit.auxiliary_attacks.gcg.extension_protocols`.
...
from pyrit.auxiliary_attacks.gcg import extension_protocols as protocols_module  # noqa: E402
...
def test_protocols_are_reexported_from_package_with_identity() -> None:
    for name in PROTOCOL_NAMES:
        ...
        assert package_attr is module_attr, (
            f"{name} re-exported from pyrit.auxiliary_attacks.gcg must be the same "
            f"object as pyrit.auxiliary_attacks.gcg.extension_protocols.{name}"
        )

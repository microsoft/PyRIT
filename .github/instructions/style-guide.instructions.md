...
`scipy`, `pandas`, `av`). This is appropriate when:

1. **CLI entry points** — defer the framework import to after arg parsing so
   `--help` is instant.
...
def main() -> int:
    ...
    from pyrit.cli import frontend_core  # heavy, deferred
    ...
...
paths stay fast — they will fail if a heavy module is loaded too early.

### Lazy `__init__.py` Exports (PEP 562)

Public API packages (`pyrit.prompt_target`, `pyrit.prompt_converter`, `pyrit.score`)
use `__getattr__`-based lazy loading so `from pyrit.score import AudioScorer` works
without paying the import cost at package load time:

```python
import importlib
from typing import TYPE_CHECKING

from mypackage.lightweight import LightClass  # eager

if TYPE_CHECKING:
    from mypackage.heavy import HeavyClass  # IDE support

_LAZY_IMPORTS: dict[str, str] = {"HeavyClass": "mypackage.heavy"}
...
Lazy names must remain in `__all__` and have a `TYPE_CHECKING` import.
...
# INCORRECT — reaching into internal files
from pyrit.prompt_target.common.prompt_chat_target import PromptChatTarget
```

Heavy submodules intentionally not re-exported from `__init__.py` are imported
directly:

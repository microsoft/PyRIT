...
#: Mirrors the compatibility key the scenario's own result-reuse cache uses, so the
#: dashboard's notion of "the same technique/model/target/scorer/dataset combination"
#: stays aligned with the scenario's own cache-reuse semantics.
...
DEFAULT_BENCHMARK_STORE_PATH = BENCHMARK_RESULTS_PATH / "adversarial_benchmark_metrics.jsonl"
